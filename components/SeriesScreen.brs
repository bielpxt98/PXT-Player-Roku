sub Init()
    ...
    m.recentEntry = { isRecent: true, category_name: "RECENTEMENTE", name: "RECENTEMENTE" }
    m.continueEntry = { isContinue: true, category_name: "CONTINUE ASSISTINDO", name: "CONTINUE ASSISTINDO" }
    m.categories = [m.favoritesEntry, m.recentEntry, m.continueEntry] : m.allSeries = [] : m.filteredSeries = [] : m.activePane = "categories" : m.categoryIndex = 0 : m.seriesIndex = 0
...
function onKeyEvent(key as String, press as Boolean) as Boolean
    ...
    if key = "left" then
        if m.activePane = "series" then m.activePane = "categories" : updateFocus()
        return true
    end if
    if key = "right" then
        if m.activePane = "categories" and m.filteredSeries.Count() > 0 then m.activePane = "series" : updateFocus()
        return true
    end if
...
function getFixedCategories() as Object
    return [m.favoritesEntry, m.recentEntry, m.continueEntry]
end function

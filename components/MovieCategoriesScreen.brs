...
sub setLoading(isLoading as Boolean)
    clearCategoryNodes()
    if isLoading then
        m.statusLabel.text = "Carregando categorias de filmes..."
...
sub setCategories(categories as Object)
    m.categories = addSearchEntry(normalizeCategories(categories), "BUSCAR FILME")
    resetSelection()

    if m.categories.Count() = 0 then
        showMessage("Nenhuma categoria de filmes foi encontrada.")
        return
    end if
